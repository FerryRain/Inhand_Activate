...
def main(config: DictConfig):
    out_dir = "/home/ferry/data/Code2/Research/Inhand_Activate/Real_deploy/results/offline_tracking/cube_obj_01/002"
    stats = TimingStats()
        ...
        while True:
            ...
            if init_done:
                with timed(stats, "stage/Manip"):
                    ...
                    # 如果还没到下一次保存时间，sleep 一下，让 CPU/磁盘喘口气（这一步通常直接解决“卡卡的”）
                    if now < next_save_t:
                        sleep_until(next_save_t)
                        now = time.perf_counter()

                    # 如果落后太多，重置节拍，避免疯狂追帧造成更卡
                    lag = now - next_save_t
