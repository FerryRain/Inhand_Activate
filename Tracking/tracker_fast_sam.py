...
from Tracking.fastsam import FastSamSegmenter, FastSAMCfg
...
class Tracker:
    def __init__(self,
                 ...
                 # -------- FastSAM params --------
                 fastsam_weights="Tracking/FastSAM-s.pt",
                 fastsam_device="cuda",
                 fastsam_imgsz=1024,
                 fastsam_conf=0.4,
                 fastsam_iou=0.9,
                 # 你希望的文本提示：
                 text_prompt="An object with colorful stickers attached",
                 # UI
                 show_tracker=True,
                 ):
        ...
        seg_cfg = FastSAMCfg(
            ...
            conf=float(fastsam_conf),
            iou=float(fastsam_iou),
            retina_masks=True,
            verbose=False,
        )
        self.segmenter = FastSamSegmenter(cfg=seg_cfg)
        print(f"[FastSAM] weights={fastsam_weights} device={fastsam_device} prompt='{self.text_prompt}'")

        # tracking params
        self.init_done = False
        self.i = 0
        self.prev_mask01 = None
        self.prev_box_xyxy = None  # 仍保留：用于显示/可选ROI门控；但不再作为 prompt
        self.last_T = None
        ...
        self.win = "Azure Kinect DK + FastSAM(text) (s:init | r:reset | q:quit)"
    ...
    def init_tracker(self):
        ...
        # 按 s：用 text prompt 初始化（不再框 ROI）
        if (key == ord('s')) and (not self.init_done):
            print(f"[INIT] FastSAM text prompt = '{self.text_prompt}'")
            ...
            try:
                ...
                print(f"[INIT] FastSAM failed: {e}")
                return

            if mask01 is None or int(mask01.sum()) == 0:
                print("[INIT] Empty mask from text prompt. Consider changing prompt.")
                return

            self.mask01 = mask01
            print(f"[INIT] FastSAM ok. infer={infer_ms:.1f}ms, mask_pixels={int(self.mask01.sum())}")
    ...
    def tracking(self):
        ...
        # --- FastSAM text prompt segmentation (no bbox prompt) ---
        try:
            t0 = time.time()
            mask01 = self.segmenter.segment_from_text(
                self.color,
            ...
            print(f"[WARN] FastSAM failed at i={self.i}: {e}")
        ...
        # print(f"[SEND] i={self.i:06d} ok={resp.get('ok')} infer_ms={infer_ms:.1f} send_cost={cost:.3f}s")
    ...
    def select_gate_mask_polygon(self, bgr, win_name="Select GATE mask (polygon)"):
        ...
        def redraw():
            vis = bgr.copy()
            cv2.putText(vis, "LMB=add vertex | u=undo | c=clear | ENTER=OK | ESC=cancel",
    ...
    def send_frame_with_sock(self,
        ...
        # roi：优先使用显式 roi；否则从 mask 自动生成（仍可保留给 bundletrack 用）
        if roi is not None:
