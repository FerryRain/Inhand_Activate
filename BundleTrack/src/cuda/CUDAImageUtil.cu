...
template<> void copy<float>(float* d_output, float* d_input, unsigned int width, unsigned int height){MLIB_CUDA_SAFE_CALL(cudaMemcpy(d_output, d_input, sizeof(float)*width*height, cudaMemcpyDeviceToDevice));}//change by guan
template<> void copy<uchar4>(uchar4* d_output, uchar4* d_input, unsigned int width, unsigned int height){MLIB_CUDA_SAFE_CALL(cudaMemcpy(d_output, d_input, sizeof(uchar4)*width*height, cudaMemcpyDeviceToDevice));}//change by guan
...
__global__ void gaussFilterDepthMapDevice(float* d_output, const float* d_input, int radius, float sigmaD, float sigmaR, unsigned int width, unsigned int height)
{
	const int x = blockIdx.x*blockDim.x + threadIdx.x;
	const int y = blockIdx.y*blockDim.y + threadIdx.y;
	...
	for (int m = x - kernelRadius; m <= x + kernelRadius; m++)
	{
		for (int n = y - kernelRadius; n <= y + kernelRadius; n++)
		{
			if (m >= 0 && n >= 0 && m < width && n < height)
			{
				...
				if (currentDepth>=0.1f && abs(currentDepth-mean_depth)<0.01)
				{
...
void gaussFilterDepthMap(float* d_output, const float* d_input, int radius, float sigmaD, float sigmaR, unsigned int width, unsigned int height)
{
	...
	gaussFilterDepthMapDevice << <gridSize, blockSize >> >(d_output, d_input, radius, sigmaD, sigmaR, width, height);
#ifdef _DEBUG
	cutilSafeCall(cudaDeviceSynchronize());
	cutilCheckMsg(__FUNCTION__);
#endif
