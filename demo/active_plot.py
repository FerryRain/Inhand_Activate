...
# ------------------------------ minimal demo -----------------------------
    ...
    def viz_2_record(
            ...
            out_path: str,
            fps: int = 20,
            duration_s: float = 6.0,  # 6 秒
            turns: float = 1.0,  # 转 1 圈
            width: int = 1280,
            height: int = 800,
            orbit_axis: str = "z",  # 围绕哪个轴旋转: "x"/"y"/"z"
            bg_rgba=(1.0, 1.0, 1.0, 1.0)  # 白色背景（alpha=1）
    ):
        """
        Record viz_2-style rendering (materials + frustum + scores + unc) into a video.
        Uses OffscreenRenderer, so no GUI window & no app.run().

        - fps=20, duration_s=6 -> 120 frames
        - turns=1 -> one full 360° orbit during those frames
        """
        import cv2
        import numpy as np
        import math
        ...
        if self.pcd is None or self.center is None or self.cam_pos is None:
            raise RuntimeError("Missing data. Call estimate() first.")
        if self.pcd_dirs is None:
            raise RuntimeError("Missing pcd_dirs. Call estimate() first.")

        out_path = str(Path(out_path).expanduser().resolve())
        Path(out_path).parent.mkdir(parents=True, exist_ok=True)
        if not (out_path.endswith(".mp4") or out_path.endswith(".avi")):
            raise ValueError(f"out_path must be a video file path (*.mp4/*.avi), got: {out_path}")

        # ---------- build offscreen scene (viz_2 style) ----------
        renderer = rendering.OffscreenRenderer(width, height)
        scene = renderer.scene
        scene.set_background(list(bg_rgba))  # 白背景

        # --- materials (copy from viz_2) ---
        mat_obj = rendering.MaterialRecord()
        mat_obj.shader = "defaultUnlit"
        mat_obj.base_color = [0.8, 0.8, 0.8, 1.0]
        ...
        # --- frustum (NBV camera frustum, static in world) ---
        extrinsic = self._get_camera_extrinsic(self.cam_pos, self.center)
        intrinsic = o3d.camera.PinholeCameraIntrinsic(width, height, 1000, 1000, width / 2.0, height / 2.0)
        frustum = o3d.geometry.LineSet.create_camera_visualization(
            view_width_px=width, view_height_px=height,
            intrinsic=intrinsic.intrinsic_matrix,
            extrinsic=extrinsic,
            scale=0.02
        )
        frustum.paint_uniform_color([1.0, 0.0, 0.2])  # green

        # --- add geometries ---
        scene.add_geometry("object_pcd", self.pcd, mat_obj)
        scene.add_geometry("direction_scores", self.pcd_dirs, mat_scores)

        if self.pcd_unc is not None:
            scene.add_geometry("uncertainty_vol", self.pcd_unc, mat_unc)

        scene.add_geometry("camera_frustum", frustum, mat_line)

        # ---------- camera setup (orbit viewer camera) ----------
        center = np.asarray(self.center, dtype=np.float64)

        bounds = self.pcd.get_axis_aligned_bounding_box()
        extent = bounds.get_extent()
        scene_radius = 0.5 * float(np.linalg.norm(extent)) + 1e-9

        fov_deg = 60.0
        aspect = float(width) / float(height)
        near = max(scene_radius * 0.01, 1e-4)
        far = scene_radius * 20.0
        ...
        # viewer initial position: put camera on +X, orbit around chosen axis
        # (这样绕 z 轴旋转才有视觉变化；如果放在 +Z，绕 z 转不会动)
        dist = (scene_radius / math.tan(math.radians(fov_deg) * 0.5)) * 2.0  # 这里等价你 viz_2 的 scale=2
        v0 = np.array([dist, 0.0, 0.0], dtype=np.float64)  # 初始相对向量

        # orbit axis
        ax = orbit_axis.lower().strip()
        if ax == "x":
        ...
        def rot_axis_angle(v, axis, theta):
        ...
        total_frames = max(1, int(round(float(duration_s) * int(fps))))

        # ---------- video writer ----------
        vw = None
        try:
            for i in range(total_frames):
                theta = 2.0 * math.pi * float(turns) * (float(i) / float(total_frames))
                eye = center + rot_axis_angle(v0, axis, theta)

                scene.camera.look_at(center, eye, up)

                img = renderer.render_to_image()
                img_np = np.asarray(img)  # uint8, shape (H,W,3) or (H,W,4)

                if img_np.ndim == 3 and img_np.shape[2] == 4:
                    img_np = img_np[:, :, :3]
                bgr = img_np[:, :, ::-1].copy()  # RGB->BGR for cv2

                if vw is None:
                    h, w = bgr.shape[:2]
                    fourcc = cv2.VideoWriter_fourcc(*("mp4v" if out_path.endswith(".mp4") else "XVID"))
                    vw = cv2.VideoWriter(out_path, fourcc, float(fps), (w, h), True)
                    if not vw.isOpened():
                        raise RuntimeError(f"Failed to open VideoWriter: {out_path} (size={w}x{h})")
            ...
            if vw is not None:
                vw.release()
            # renderer release（不同版本 open3d 可能没有 release()，所以 try-catch）
            try:
                renderer.release()
            except Exception:
                pass

        print(f"[OK] Saved: {out_path}  (fps={fps}, duration={duration_s}s, turns={turns})")
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--in_path",
        type=str,
        default="/home/ferry/data/Code2/Research/Inhand_Activate/demo/results/cube_obj_01/002/pcd_online/recon_0_000587.ply"
    )
    ...
    if not args.no_viz:
        est.viz_2_record(
            out_path="/home/ferry/data/Code2/Research/Inhand_Activate/demo/results/cube_obj_01/002/active/29s.mp4",
            fps=20,
            duration_s=6.0,  # 6s
            turns=1.0,  # 转一圈
            orbit_axis="z",  # 想绕别的轴可改成 "x"/"y"
            bg_rgba=(1.0, 1.0, 1.0, 1.0)  # 白背景
        )
